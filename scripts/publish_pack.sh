...
if [[ "${SKIP_BUMP:-}" != "1" ]]; then
  ...
  python3 variants/cel-band/scripts/pack_version.py --bump-patch
fi

VERSION="$(python3 variants/cel-band/scripts/pack_version.py --print semver)"
TAG="$(python3 variants/cel-band/scripts/pack_version.py --print tag)"
LABEL="$(python3 variants/cel-band/scripts/pack_version.py --print label)"
...
git add variants/cel-band/VERSION dist/ download/
...
git tag -a "${TAG}" -m "Cel Band Pack ${TAG}"

git push origin HEAD:main
git push origin "${TAG}"

gh release create "${TAG}" \
  --title "Cel Band Pack ${TAG}" \
  --notes "Install \`Cel_Band_Pack.mcpack\` on Minecraft Bedrock 1.21+.
